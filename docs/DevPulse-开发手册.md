...
|------|---------|--------|--------|------|
| accessToken | `{ sub: userId, email, tokenVersion }` | `JWT_SECRET` | 15min | 前端 localStorage |
| refreshToken | `{ sub: userId, deviceId: uuid }` | `JWT_REFRESH_SECRET` | 7d | 前端 localStorage + HttpOnly Cookie 双通道；后端 Redis `rt:{userId}:{deviceId}` HASH (bcrypt hash + 设备元数据, TTL 7d) |
...
**Token 存储选择 `localStorage` 而非 `sessionStorage`：** `sessionStorage` 是标签页隔离的，新开标签页无法共享登录态。`localStorage` 同域名下所有标签页共享，配合 `storage` 事件实现跨标签页同步（一个标签页登出，其他标签页自动感知并提示）。安全性方面，accessToken 有效期仅 15 分钟且通过 HttpOnly Cookie 传输 refreshToken，XSS 风险可控。
...
前端 Axios 配置 `withCredentials: true`，浏览器自动携带 HttpOnly Cookie。refresh 拦截器仍保留 localStorage 中的 refreshToken 作为兜底（兼容 Cookie 未设置的场景）。
...
4. **认证流程**：`LoginPage` → `RegisterPage`（登录成功后 token 存入 localStorage）
...
采用 SWR（Stale-While-Revalidate）模式管理用户认证状态：页面加载时先用本地 `localStorage` 缓存立即渲染（零延迟），后台通过 TanStack Query 的 `useQuery` 静默调用 `GET /auth/me` 获取最新用户数据。配置 `staleTime: 5min` 避免频繁请求，`enabled: isAuthenticated` 确保未登录状态不发请求。
...
### 16.10 Zustand localStorage 持久化 + resolveUploadUrl 兼容

使用 Zustand 将用户认证状态序列化到 `localStorage`，刷新页面后从缓存恢复状态避免重新登录。选择 `localStorage` 而非 `sessionStorage` 的原因：`sessionStorage` 是标签页隔离的，新开标签页无法共享登录态，用户体验差；`localStorage` 同域名下所有标签页共享，配合 `storage` 事件实现跨标签页同步。`resolveUploadUrl()` 工具函数智能判断 URL 类型：完整 URL 直接返回，相对路径自动拼接后端域名，兼容不同部署阶段的数据格式。
...
初始实现中 Zustand store 只持久化了 `isAuthenticated` 标志和 token，`user` 对象未持久化。刷新页面后 `isAuthenticated` 为 `true` 但 `user` 为 `null`，导致页面头部显示"未登录"状态而实际已登录。修复方案是将 `user` 对象一并序列化到 `localStorage`，并通过 `partialize` 配置只持久化必要的用户字段（id、username、avatar、role），避免存储过多数据。

### 17.7 CORS 多域名逗号分隔导致浏览器拒绝

**现象：** 生产环境登录接口返回 HTTP 200，但前端拿不到响应数据，登录态无法保存。浏览器 Console 无明显报错（CORS 失败时浏览器直接拦截，不会进入 JS）。

**根因：** `FRONTEND_URL` 配置为 `http://ip,http://domain,https://domain`，NestJS CORS 中间件原样输出 `Access-Control-Allow-Origin: http://ip,http://domain,https://domain`。但 CORS 规范要求 `Access-Control-Allow-Origin` 只能是单个源或 `*`，逗号分隔的值浏览器直接视为不匹配，拒绝所有跨域请求。

**修复：** 在 `main.ts` 中解析 `FRONTEND_URL` 为数组，根据请求的 `Origin` 头动态匹配并返回单个值：
...
**面试话术：** "CORS 的 `Access-Control-Allow-Origin` 响应头只接受单个源或 `*`，不支持逗号分隔。多域名场景必须动态匹配请求 Origin 后逐个返回。这是部署到生产环境后最容易踩的坑——本地开发时前端和后端同源（Vite proxy），CORS 问题不会暴露。"

### 17.8 Cookie secure 标志在 HTTP 下导致登录态丢失

**现象：** 用户登录成功后关闭浏览器再打开，登录态消失。检查发现 Cookie 中没有 `refresh_token`。

**根因：** 后端 `CookieOptions` 硬编码 `secure: true`，表示 Cookie 只在 HTTPS 连接下发送。生产环境备案未完成，使用 `http://ip` 访问，浏览器直接忽略带 `secure` 标志的 Set-Cookie 头，refreshToken 存不进去。

**修复：** 根据 `FORCE_HTTPS` 环境变量动态设置 `secure`：

```typescript
const isHttps = process.env.FORCE_HTTPS === 'true';
res.cookie('refresh_token', token, {
  secure: isHttps,  // HTTP 下为 false，HTTPS 下为 true
...
**面试话术：** "Cookie 的 `secure` 标志要求 HTTPS 连接才发送。部署初期用 HTTP 访问时，`secure: true` 的 Cookie 会被浏览器静默丢弃。最佳实践是根据协议动态设置，HTTP 环境用 `lax`，HTTPS 环境用 `secure + sameSite:none`。"

### 17.9 React 渲染期间 setState 导致无限循环

**现象：** 登录后页面卡死，Console 疯狂报错 `Cannot update a component (MainLayout) while rendering a different component (MainLayout)`。

**根因：** `useAuthRefresh` Hook 中直接在函数体调用 `setUser(data)`，而函数体在 React 渲染阶段执行。React 不允许在渲染期间触发状态更新，会导致无限循环：渲染 → setState → 重新渲染 → setState → ...

**修复：** 将状态更新移入 `useEffect`，确保在渲染完成后异步执行：
...
**面试话术：** "React 的渲染阶段是纯函数，不能有副作用。`useQuery` 返回的 `data` 在渲染期间就可用了，但更新 store 必须放在 `useEffect` 中。这是 React 18 并发模式下的强制要求，否则会触发 `setState-in-render` 错误。"

### 17.10 sessionStorage 标签页隔离导致多标签页登录态不共享

**现象：** 在标签页 A 登录后，新开标签页 B 显示未登录状态。标签页 A 退出登录后，标签页 B 仍显示已登录但所有 API 请求失败。

**根因：** `sessionStorage` 是标签页隔离的，每个标签页有独立的存储空间，无法跨标签页共享数据。

**修复：** 两步改造：

1. **`sessionStorage` → `localStorage`**：`localStorage` 同域名下所有标签页共享，新标签页可直接读取登录态。

2. **`storage` 事件跨标签页同步**：监听 `window.storage` 事件，当一个标签页登出（清除 localStorage），其他标签页自动感知并提示：
...
window.addEventListener('storage', (e) => {
  ...
  if (e.key === 'user' && e.newValue) {
    store.setUser(JSON.parse(e.newValue));
  }
});
```

**面试话术：** "`sessionStorage` 是标签页隔离的，`localStorage` 是同源共享的。多标签页同步的标准做法是：用 `localStorage` 存储登录态 + 监听 `storage` 事件。当一个标签页登出时清除 localStorage，其他标签页通过 `storage` 事件感知到变化，自动同步状态并提示用户。GitHub 和 Notion 都采用这个方案。"

### 17.11 PostgreSQL 全文搜索中英文分词策略
...
### 17.12 Docker 多阶段构建中 Prisma generate 的时机
