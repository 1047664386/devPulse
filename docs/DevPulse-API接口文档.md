...
| POST | `/auth/forgot-password` | 否 | 忘记密码（发送重置邮件） |
| POST | `/auth/reset-password` | 否 | 重置密码（验证令牌+设置新密码） |
...
实现要点:
- **防邮箱枚举**：邮箱不存在时返回与成功相同的消息，不泄露用户注册信息。
- **冷却机制**：同一邮箱 60 秒内只能发送一次重置邮件（Redis key: `pwd_reset_cd:{email}`），超频返回 `ErrResetCooldown(20027)`。邮件发送失败时不设冷却期，允许用户立即重试。
- **邮件发送失败处理**：同步 await 邮件发送结果，失败时返回 `ErrMailSendFailed(20028)` + 不设冷却期，用户可立即重试。邮箱不存在时仍设冷却期（防频率枚举）。
- 生成 JWT 重置令牌（payload: `{sub: userId, purpose: 'password-reset'}`, 有效期 30 分钟）。
- 令牌状态存入 Redis (`pwd_reset:{userId}` → `"unused"`, TTL=30分钟)。
- 构造前端重置链接 `{FRONTEND_URL}/reset-password?token={resetToken}`，通过 MailService 发送邮件。
- 邮件服务两种模式（**无论哪种模式，SMTP 发送失败都会抛错**，由本接口 catch 后返回 `ErrMailSendFailed(20028)` + 不设冷却期）：
  - (1) 开发模式：`SMTP_HOST` 为 `127.0.0.1`/localhost 或无认证信息 → 连接本地 Mailpit（启动：`docker compose up -d mailpit`，查看邮件：`http://localhost:8025`）。发送前会把重置链接打印到终端日志辅助调试，但 SMTP 连接失败仍会抛错（不再静默吞掉）。**`SMTP_HOST` 务必用 `127.0.0.1`**，避免 macOS 下 `localhost` 被解析为 IPv6 `::1` 导致 `ECONNREFUSED ::1:1025`。
  - (2) 生产模式：配齐 `SMTP_HOST` + `SMTP_USER` + `SMTP_PASS` → 真实 SMTP 发送，失败时抛错。

**POST /auth/reset-password**

请求体:
```json
{ "token": "eyJhbGci...", "newPassword": "NewP@ss123!" }
```
校验规则: token 非空，newPassword 8+位含大小写和数字。
...
实现要点:
- 校验 JWT 令牌签名与有效期 → 过期/无效返回 `ErrResetTokenExpired(20024)` 或 `ErrResetTokenInvalid(20025)`。
- 校验令牌 `purpose` 字段为 `"password-reset"` → 否返回 `ErrResetTokenInvalid(20025)`。
- Redis 查询令牌状态：不存在→`ErrResetTokenExpired(20024)`；值为 `"used"`→`ErrResetTokenUsed(20026)`。
- 使用一次后立即标记为 `"used"` → 防止重放攻击。
- 更新密码（bcrypt saltRounds=12）。
- **安全联动**：递增 `tokenVersion` + 清除全部设备 Redis 会话 → 强制所有设备重新登录。
...
> URL 格式说明：当后端配置了 `APP_URL`（如 `https://api.example.com`）时，返回完整 URL `https://api.example.com/uploads/abc123.webp`；未配置时返回相对路径 `/uploads/abc123.webp`。前端 `resolveUploadUrl()` 兼容两种格式。
